...
def test_onednn_flash_tf_attention_matches_tensorflow_reference() -> None:
    ...
    actual = onednn_flash_attention(q, k, v, causal=True, query_block_size=3, key_block_size=4)
...
def test_onednn_flash_tf_attention_qk_layouts_match_reference() -> None:
...
def test_onednn_flash_tf_attention_brgemm_transformed_k_matches_copied() -> None:
    brgemm_op_path = Path("build-brgemm-main/onednn_flash_tf_attention.so").resolve()
    ...
    # OneDNNFlashAttention in the same Python process. Run the BRGEMM custom-op
    # smoke in a subprocess so the default-op tests stay isolated.
    script = f"""
import numpy as np
import tensorflow as tf
from onednn_flash_tf import onednn_flash_attention
...
copied = onednn_flash_attention(q, k, v, qk_tile_layout='copied_transposed', **common)
transformed = onednn_flash_attention(q, k, v, qk_tile_layout='brgemm_transformed_k', **common)
...
def test_onednn_flash_tf_attention_default_tile_heuristic_matches_reference() -> None:
    ...
    expected = tf_reference_attention(q, k, v, causal=True)
    actual = onednn_flash_attention(q, k, v, causal=True)
    np.testing.assert_allclose(actual.numpy(), expected.numpy(), rtol=1e-5, atol=1e-5)
