...
void test_case(bool causal, std::size_t block_size) {
    const flashone::AttentionShape shape{9, 11, 7, 5};
    ...
    const auto actual = flashone::flash_attention_tiled(q, k, v, shape, options);
    require_close(flashone::max_abs_diff(expected, actual), 1e-5f, causal ? "causal" : "dense");
}
...
int main() {
    ...
    test_case(true, 4);
    test_case(true, 64);
    std::cout << "flashone C++ attention tests passed\n";
    return 0;
}
