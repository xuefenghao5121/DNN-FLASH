...
void require_close(const std::vector<float>& expected,
                   ...
                   float tolerance,
                   const char* label) {
    const float diff = flashone::max_abs_diff(expected, actual);
    if (diff > tolerance) {
...
flashone::BlockMask make_window_mask(std::size_t query_tokens,
...
void test_ws_matches_standard(flashone::TileKernelKind qk_kind,
    ...
    const auto expected = flashone::standard_attention(q, k, v, shape, options);

    std::vector<float> output(shape.query_tokens * shape.value_dim, 0.0f);
    flashone::AttentionWorkspace ws;
    flashone::flash_attention_qk_pv_tile_ws(q.data(), k.data(), v.data(), output.data(),
                                            shape, options, ws);
...
void test_ws_reuse() {
    ...
    const flashone::AttentionShape shape{16, 16, 32, 32};
    ...
    options.qk_tile_kernel = flashone::TileKernelKind::Reference;
    options.pv_tile_kernel = flashone::TileKernelKind::Reference;
    ...
    flashone::flash_attention_qk_pv_tile_ws(q.data(), k.data(), v.data(), out1.data(),
                                            shape, options, ws);
    flashone::flash_attention_qk_pv_tile_ws(q.data(), k.data(), v.data(), out2.data(),
                                            shape, options, ws);
...
int main() {
    test_ws_matches_standard(flashone::TileKernelKind::Reference,
                             flashone::TileKernelKind::Reference);
    test_ws_matches_vector_api(flashone::TileKernelKind::Reference,
                               flashone::TileKernelKind::Reference);
    test_ws_reuse();
