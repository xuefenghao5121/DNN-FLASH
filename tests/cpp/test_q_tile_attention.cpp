...
flashone::BlockMask make_band_mask(std::size_t query_tokens,
...
void test_q_tile_matches_standard(flashone::TileKernelKind kernel_kind) {
    const flashone::AttentionShape shape{10, 13, 6, 5};
    ...
    const auto actual = flashone::flash_attention_q_tile(q, k, v, shape, options);
    require_close(expected, actual, 1e-5f, flashone::tile_kernel_name(kernel_kind));
}

}  // namespace

int main() {
    test_q_tile_matches_standard(flashone::TileKernelKind::Reference);
#ifdef FLASHONE_HAS_ONEDNN
    test_q_tile_matches_standard(flashone::TileKernelKind::OneDnn);
#endif
    std::cout << "flashone Q-tile attention tests passed\n";
    return 0;
}
