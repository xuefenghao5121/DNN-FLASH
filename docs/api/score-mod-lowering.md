...
ScoreMod Lowering 模块负责把用户侧 FlexAttention 的 `score_mod` 语义转换成 OneDNN-Flash Runtime 与 oneDNN 后端可执行的结构化计划。
...
ScoreModPlan:
  lowered_ops:
    - op kind
    - backend target: onednn_post_ops | onednn_flash_epilogue | reference_callback
...
3. OneDNN-Flash epilogue fallback。
...
if oneDNN primitive path supports scale attr for current dtype/layout:
    ...
    lower to OneDNN-Flash epilogue
```
...
3. OneDNN-Flash epilogue add fallback。
...
ALiBiSpec
  -> BiasTileGenerator
  -> additive_bias plan
  -> oneDNN binary add or OneDNN-Flash epilogue
```
...
- 若 oneDNN post-op 不适合表达，则 OneDNN-Flash epilogue fallback。
