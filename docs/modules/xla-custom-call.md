...
XLA Custom Call 是 FlashOne 区别于普通 TensorFlow Custom Op 的关键路径。

目标：

> 保持 FlexAttention pattern 不被 XLA 拆成 materialized `QK -> softmax -> PV`，而是作为整体 lowering 到 FlashOne Runtime。
...
TensorFlow function
  -> XLA HLO
  -> FlashOne CustomCall HLO
  -> XLA runtime calls flashone_attention_entry
  -> RuntimePlanner
...
```text
flashone_attention_entry(
  void* out,
  const void* q,
  const void* k,
  const void* v,
  const FlashOneOpaqueConfig* config,
  size_t config_size
)
```
...
- CustomCall opaque 中包含 FlashOne config。
