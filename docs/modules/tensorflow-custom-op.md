...
TensorFlow Custom Op 是 FlashOne 在 XLA Custom Call 完成前的工程验证路径，同时也是非 XLA 环境下的 fallback runtime。
...
当前已有 `FlashOneAttention` CPU op，支持：
...
- Q/K/V 只读，不复制。
- O 由 TensorFlow allocate_output，一次分配，FlashOne direct write。
- 临时 workspace 由 FlashOne 管理，不通过 TensorFlow 暴露。
