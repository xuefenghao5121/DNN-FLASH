...
using MatmulCacheKey = std::tuple<std::size_t, std::size_t, std::size_t,
...
struct OneDnnTileContext {
    ...
    std::mutex mutex;
    std::map<MatmulCacheKey, CachedMatmulPrimitive> cache;
};
...
CachedMatmulPrimitive make_cached_primitive(dnnl::engine& engine,
    ...
    auto a_md = dnnl::memory::desc(
        ...
        {static_cast<dnnl::memory::dim>(shape.a_stride_m),
         static_cast<dnnl::memory::dim>(shape.a_stride_k)});
    auto b_md = dnnl::memory::desc(
        {k, n},
        dnnl::memory::data_type::f32,
        {static_cast<dnnl::memory::dim>(shape.b_stride_k),
         static_cast<dnnl::memory::dim>(shape.b_stride_n)});
    auto c_md = dnnl::memory::desc(
        {m, n},
        dnnl::memory::data_type::f32,
        {static_cast<dnnl::memory::dim>(shape.c_stride_m),
         static_cast<dnnl::memory::dim>(shape.c_stride_n)});
    auto pd = dnnl::matmul::primitive_desc(engine, a_md, b_md, c_md);
    return CachedMatmulPrimitive{a_md, b_md, c_md, dnnl::matmul(pd)};
}

StridedMatmulShape contiguous_shape(const MatmulShape& shape) {
    return StridedMatmulShape{
        shape.m,
        shape.n,
        shape.k,
        shape.k,
...
MatmulCacheKey cache_key(const StridedMatmulShape& shape) {
    return {
        shape.m,
        shape.n,
        shape.k,
...
void matmul_tile_onednn_strided_inplace(const float* a,
                                        const float* b,
                                        float* c,
                                        const StridedMatmulShape& shape);
...
void matmul_tile_onednn_inplace(const float* a,
                                 ...
                                 float* c,
                                 const MatmulShape& shape) {
    matmul_tile_onednn_strided_inplace(a, b, c, contiguous_shape(shape));
}
...
void matmul_tile_onednn_strided_inplace(const float* a,
    ...
    CachedMatmulPrimitive* cached = nullptr;
    {
